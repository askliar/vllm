...
"""ModelOpt -> AnyModel config adapter.

Legacy NAS/ModelOpt checkpoints describe per-layer overrides via a dense
``block_configs`` list with nested ``attention`` / ``ffn`` sub-sections.
AnyModel now consumes the HuggingFace heterogeneity schema: a sparse
``per_layer_config`` dict mapping ``layer_idx -> {flat HF keys + optional
"skip" list}``.

This module rewrites the legacy schema in-place so downstream code only
...
def convert_block_configs_to_per_layer_config(hf_config: Any) -> bool:
    ...
    moe_num_field, moe_size_field = _MOE_FIELDS_BY_ARCH.get(
        base_architecture, _DEFAULT_MOE_FIELDS
    )
