...
def test_compile_creation_failure_disables_fast_path(monkeypatch):
    x = torch.randn(4, 8, dtype=torch.bfloat16)
    w = torch.randn(16, 8, dtype=torch.bfloat16)

    def raise_compile_failure(_x, _weight, _bias, _allow_new_compile):
        raise RuntimeError("compile warmup boom")
