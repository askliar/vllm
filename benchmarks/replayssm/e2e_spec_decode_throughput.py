...
FlashInfer autotuning is enabled by default so ReplaySSM is tuned before CUDA
graph capture. It can be disabled explicitly for debugging.
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=False,
        help="Disable FlashInfer autotuning (default: off).",
    )
    p.add_argument(
        "--modes",
...
def run_worker(args):
    ...
    llm_kwargs = dict(
        ...
        seed=0,
        mamba_backend=args.replayssm_backend,
...
def main():
    ...
    if "cache" in results and "spec" in results:
        if results["cache"]["output_digest"] != results["spec"]["output_digest"]:
            raise AssertionError("ReplaySSM and native spec produced different tokens")
        print(f"native-spec token parity : {results['cache']['output_digest']}")
        print(
